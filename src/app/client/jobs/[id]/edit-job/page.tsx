...
import { Briefcase, ArrowLeft, DollarSign, FileText, Tag, X, Plus, AlertCircle, CheckCircle } from "lucide-react"
...
export default function EditJobPage() {
 ...
 async function fetchJob(){
    try {
        const res = await axios.get(`/api/jobs/client/job/${jobId}`)
        if(!res) router.push(`/client/dashboard/${session?.user.id}`)
  ...
  const handleSubmit = async () => {
    ...
    try {
         setIsLoading(true)
      const response = await axios.put(`/api/jobs/client/job/${jobId}/edit-job`, formData)
      ...
      if (response.data.error) {
        ...
        setErrors({ general: data.error || "Failed to update job" })
      }
      else{
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Edit the Job</h1>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <Button
                ...
                size="lg"
              >
                {isLoading ? "Updating job..." : "Edit Job"}
              </Button>
